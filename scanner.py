...
from database import (
    ...
    update_trade_note,
    update_stop_loss,
    save_trade_analytics  # Added as per Step 1 & 2
)
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO active_signals (
                ...
                target1, target2, score, rr, quantity
            )
    ...
    for trade in trades:
        try:
            ...
            if current_price >= float(target2):
                ...
                # Step 3: Save analytics before closing
                ...
                # Cleanup states
                for state_dict in [LAST_ALERT_STATE, TRADE_STATE, BREAKEVEN_DONE]:
            ...
            if current_price <= float(stop_loss):
                ...
                # Step 4: Save analytics before closing
                save_trade_analytics(
                    symbol=symbol,
                    result="SL_HIT",
                    entry_price=entry_price,
...
def run_scanner():
    if SCAN_LOCK["running"] or not is_market_hours():
        return
        ...
        for symbol in batch:
            try:
                ...
                if result:
                    save_signal(result)
                    send_alert(
                        symbol=result['symbol'], action="BUY", entry=result['entry'],
                        sl=result['sl'], target1=result['target1'], target2=result['target2'],
                        confidence=result['score'], reason=result['reasons'],
