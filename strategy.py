...
def analyze_setup(df, symbol):
    try:
        ...
        # =========================================
        htf_df = get_higher_timeframe_data(symbol)
        if htf_df is None or len(htf_df) < 50:
            logger.info(f"HTF data unavailable for {symbol}")
            return None
        ...
        higher_tf_bullish = (htf_latest['ema_50'] > htf_latest['ema_200'])
        
        if not higher_tf_bullish:
            logger.info(f"{symbol} rejected by HTF filter (EMA 50 < 200)")
            return None
        ...
        strong_bullish_candle = (candle_body > (0.5 * candle_range) and upper_wick < (0.3 * candle_range))

        regime, atr_multiplier = detect_market_regime(df)
        avg_volume = df['volume'].rolling(window=10).mean().iloc[-1]

        score = 20 # Starting with Nifty OK score
        reasons = ["Nifty OK"]

        if (prev['close'] < prev['vwap'] and latest['close'] > latest['vwap'] and 
            latest['volume'] > avg_volume and strong_bullish_candle and structure_breakout):
            score += 20
        ...
        if (latest['close'] > latest['ema_20'] > latest['ema_50']):
            if ema_distance < 0.003: return None
            score += 15
        ...
        if personality_score != 0: reasons.append(f"History {personality_score:+d}")

        sector_score = get_sector_strength(symbol)
        score += sector_score
        if sector_score != 0: reasons.append(f"Sector {sector_score:+d}")

        atr, entry = latest['atr'], latest['close']
