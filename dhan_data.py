...
def get_dhan_data(symbol):
    try:
        ...
        if isinstance(candles, dict) and "errorCode" in candles:
            logger.error(f"Dhan API error for {symbol}: {candles.get('errorMessage')} | Code: {candles.get('errorCode')}")
            return None
        ...
        # Dict of lists format
            ...
            logger.warning(f"Unhandled candles format for {symbol}: {type(candles)}")
            return None
